...
class ResnetBlock(Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=None, condition_emb_dim=None, dropout=0.):
        ...
        assert time_emb_dim is None and condition_emb_dim is not None, "Time embedding is not supported for pure UNet usage"
        emb_dim = int(condition_emb_dim)

        self.mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear(emb_dim, dim_out * 2)
        ) if emb_dim > 0 else None
...
class Unet1DWithoutTime(Module):
    def __init__(
            self,
            dim,
            init_dim=None,
        ...
        resnet_block = partial(ResnetBlock, time_emb_dim=None, condition_emb_dim=self.embed_y_all_dim, dropout=dropout)
    ...
    def forward(self, x, c, cond_drop_prob = None):
        ...
        # TODO: time embedding is not supported for pure UNet usage, so we set it to None
        t = None
