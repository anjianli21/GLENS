...
class Trainer(object):
    def __init__(
        ...
        if self.accelerator.is_main_process:
            ...
            if self.use_wandb:
                run_name = (
                    ...
                    f"trainig_steps_{model_params['training_steps_limit']}"
                )
                wandb.init(
                    ...
                    resume='allow',  # Allow resuming from a previous run
                    id=previous_run_id if previous_run_id is not None else None  # Conditionally set the run ID
                )
        ...
        self.val_loss_history = deque(maxlen=smoothing_window)  # Use deque with maxlen
        ...
        with tqdm(initial=self.step, total=self.train_num_steps, disable=not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                if self.warmup_steps > 0 and self.step < self.warmup_steps:
                    warmup_factor = float(self.step + 1) / float(self.warmup_steps)
                    target_lr = self.base_lr * warmup_factor
                    for param_group in self.opt.param_groups:
                        param_group['lr'] = target_lr
                elif self.warmup_steps > 0 and self.step == self.warmup_steps:
                    # ensure we restore exact base lr once warmup completes
                    for param_group in self.opt.param_groups:
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step % self.step_per_epoch == 0 and self.step != 0:
                        ...
                        # Save the best checkpoint if this validation loss is better
    ...
    def compute_moving_average(self):
        # Compute the moving average using the current contents of the deque
        return np.mean(self.val_loss_history)

    def save_best_checkpoint(self, val_loss, milestone):
        # Append the current validation loss to history
        self.val_loss_history.append(val_loss)

        # Compute the moving average of the validation loss
        smoothed_val_loss = self.compute_moving_average()

        if smoothed_val_loss < self.best_val_loss:
            # Save the new best checkpoint with generic name
            checkpoint_name = f"model-best_validation"
            self.save(checkpoint_name)
            
            # Save a milestone-specific version
    ...
    def compute_validation_loss(self):
        ...
        with torch.no_grad():
            for batch in self.val_dl:
                val_loss = self.ema.ema_model(batch=batch, curr_epoch_num=self.curr_epoch_num)  # Use EMA model here
