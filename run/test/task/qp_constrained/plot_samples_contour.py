...
For the diamond one-sided lower-bound QP (see ``compute_k_neighbor.py``), each panel can
overlay line-only contours of
``0.5(x_1^2+x_2^2) + \\alpha\\ln(1+\\exp(-x_1-x_2))`` with ``\\alpha`` = first condition
parameter, plus the boundary line ``x_1+x_2=0`` in slice coordinates (on by default).
With that overlay, axis limits are a square ``[L,L+S]^2`` with fixed ``L`` (default ``-0.5``)
and ``S`` from sample maxima (GT plus all test paths) plus margins.
...
Use ``--x_label`` / ``--y_label`` for axis names when coordinates are not ``x_1,x_2``.
"""
import argparse
import glob
import os
...
def _collect_slice_xy_points(
    ...
    """All horizontal/vertical slice coordinates from GT (selected seeds) and every loaded test group."""
    xs_l: List[np.ndarray] = []
    ys_l: List[np.ndarray] = []
    for seed in condition_seeds:
        if seed not in gt_data:
            continue
        gt_xs = gt_data[seed]["xs"]
        if gt_xs.ndim != 2 or gt_xs.shape[1] <= max(x_dim1, x_dim2):
            continue
        pts = gt_xs[:, [x_dim1, x_dim2]]
        xs_l.append(np.asarray(pts[:, 0], dtype=float).reshape(-1))
        ys_l.append(np.asarray(pts[:, 1], dtype=float).reshape(-1))
    for test_groups in test_methods_data:
        for xs in test_groups.values():
            if xs.ndim != 2 or xs.shape[1] <= max(x_dim1, x_dim2):
                continue
            pts = xs[:, [x_dim1, x_dim2]]
            xs_l.append(np.asarray(pts[:, 0], dtype=float).reshape(-1))
            ys_l.append(np.asarray(pts[:, 1], dtype=float).reshape(-1))
    if not xs_l:
...
def _expand_span_until_square_intersects_u_plus_v_zero(floor: float, span: float) -> float:
    """
    For axis square ``[floor, floor+span]^2``, increase ``span`` (keeping ``floor``) until the
    box intersects ``u+v=0`` with a non-degenerate segment.
    """
    eps = 1e-6
    for _ in range(32):
        s_mid = 2.0 * floor + span
        half = 0.5 * span
        need_half = 0.5 * abs(s_mid) + eps
        if half + 1e-15 >= need_half:
            return span
        span = 2.0 * need_half
    return span


def compute_square_axis_limits_fixed_lower_floor(
    gt_data: Dict[int, Dict[str, np.ndarray]],
    test_methods_data: List[Dict[Tuple[float, ...], np.ndarray]],
    condition_seeds: List[int],
    x_dim1: int,
    x_dim2: int,
    lower_floor: float,
    upper_margin_frac: float,
    axis_expand_scale: float,
) -> Tuple[float, float, float, float]:
    """
    Square view ``[lower_floor, lower_floor+S]^2``: upper ``S`` from sample maxima (GT + tests)
    plus a relative margin, then ``axis_expand_scale`` on the span (corner pinned at
    ``(lower_floor, lower_floor)``), then any extra span so ``u+v=0`` is visible.
    """
    px, py = _collect_slice_xy_points(gt_data, test_methods_data, condition_seeds, x_dim1, x_dim2)
    if px.size == 0:
        span = 1.0
    else:
        span = max(float(np.max(px)) - lower_floor, float(np.max(py)) - lower_floor, 1e-8)
        span *= 1.0 + float(upper_margin_frac)
    if axis_expand_scale and axis_expand_scale > 1.0:
        span *= float(axis_expand_scale)
    span = _expand_span_until_square_intersects_u_plus_v_zero(lower_floor, span)
    hi = lower_floor + span
    return lower_floor, hi, lower_floor, hi


def _softplus_np(z: np.ndarray) -> np.ndarray:
    """Numerically stable softplus(z) = ln(1 + exp(z)), element-wise."""
    z = np.asarray(z, dtype=float)
    return np.log1p(np.exp(-np.abs(z))) + np.maximum(z, 0.0)


def objective_diamond_one_lower_bound_2d(x1: np.ndarray, x2: np.ndarray, alpha: float) -> np.ndarray:
    """
    2D slice objective matching ``compute_k_neighbor`` for the first pair:
    0.5 * (x1^2 + x2^2) + alpha * ln(1 + exp(-x1 - x2)).
    """
    return 0.5 * (x1 * x1 + x2 * x2) + float(alpha) * _softplus_np(-(x1 + x2))


def _segment_x_plus_y_zero_in_box(
    x_min: float, x_max: float, y_min: float, y_max: float
) -> Optional[Tuple[Tuple[float, float], Tuple[float, float]]]:
    """
    Return two endpoints of the segment ``{(x,y): x+y=0}`` clipped to the axis-aligned box,
    or None if the line misses the box (should not happen for sensible limits).
    """
    candidates = [
        (x_min, -x_min),
        (x_max, -x_max),
    ...
    if not pts:
        return None
    # Deduplicate near-duplicate corners
    uniq = []
    for p in pts:
    ...
    if len(uniq) < 2:
        return None
    # Farthest pair among uniq (at most 4 points)
...
def plot_qp_diamond_objective_contours(
    ax,
    x_min: float,
    x_max: float,
    y_min: float,
    ...
    contour_color: str = "0.35",
    contour_linewidth: float = 0.9,
):
    """Line-only contours of the 2D slice objective (``x_1``, ``x_2`` / first pair)."""
    gx = np.linspace(x_min, x_max, 220)
    gy = np.linspace(y_min, y_max, 220)
    X, Y = np.meshgrid(gx, gy)
    Z = objective_diamond_one_lower_bound_2d(X, Y, alpha)
    zmin, zmax = float(np.nanmin(Z)), float(np.nanmax(Z))
    if zmax > zmin and contour_levels > 0:
...
# Constraint u+v=0: strong blue, distinct from gray contours and ``_METHOD_COLORS_HEX``.
_CONSTRAINT_LINE_COLOR = "#0072B2"


def plot_slice_sum_zero_constraint(
    ax,
    x_min: float,
    x_max: float,
    y_min: float,
    y_max: float,
    constraint_color: str = _CONSTRAINT_LINE_COLOR,
    constraint_linewidth: float = 1.5,
    constraint_linestyle: str = "--",
):
    """Plot ``u + v = 0`` in panel coordinates ``(u, v)`` (clipped to the axis box)."""
    seg = _segment_x_plus_y_zero_in_box(x_min, x_max, y_min, y_max)
    if seg is None:
        return
    (xa, ya), (xb, yb) = seg
    ax.plot(
        ...
        linestyle=constraint_linestyle,
        alpha=0.9,
        zorder=2,
...
def plot_overlay_cell(
    ...
    alpha: float,
    show_objective_contour: bool,
    show_sum_zero_constraint: bool,
):
    """
    One condition (row) × one test method (column): optional objective contours + ``u+v=0`` line,
    GT as circles, matched test samples as crosses on top (method_color).

    Objective contours use ``alpha`` = first entry of ``condition_lambda_value`` (same as
    ``lambda_1`` in ``compute_k_neighbor.py``) and are only drawn for the ``(x_1, x_2)`` slice
    ``x_dim1=0``, ``x_dim2=1``. The constraint line is drawn in panel coordinates (horizontal +
    vertical axis value = 0).
    """
    if show_objective_contour:
        plot_qp_diamond_objective_contours(ax, x_min, x_max, y_min, y_max, alpha)
    if show_sum_zero_constraint:
        plot_slice_sum_zero_constraint(ax, x_min, x_max, y_min, y_max)
...
def plot_gt_and_test_methods(
    ...
    xlabel: str = r"$x_1$",
    ylabel: str = r"$x_2$",
    qp_diamond_overlay: bool = True,
    axis_expand_scale: float = 1.08,
    axis_lower_floor: float = -0.5,
    axis_upper_margin_frac: float = 0.05,
):
    """
    ...
    With ``qp_diamond_overlay``, both axes use a fixed lower limit ``axis_lower_floor``
    (default ``-0.5``) and a common upper limit set from the max over GT and test samples.
    """
    ...
    if qp_diamond_overlay:
        x_min, x_max, y_min, y_max = compute_square_axis_limits_fixed_lower_floor(
            gt_data,
            test_methods_data,
            seeds_in_order,
            x_dim1,
            x_dim2,
            lower_floor=axis_lower_floor,
            upper_margin_frac=axis_upper_margin_frac,
            axis_expand_scale=max(float(axis_expand_scale), 1.0),
        )
    else:
    ...
    show_objective_contour = qp_diamond_overlay and x_dim1 == 0 and x_dim2 == 1
    show_sum_zero_constraint = bool(qp_diamond_overlay)
    ...
    for r, seed in enumerate(seeds_in_order):
        lam0 = np.asarray(gt_data[seed]["lambda"], dtype=float).reshape(-1)
        alpha_row = float(lam0[0]) if lam0.size else 0.0
        for c in range(n_methods):
            ...
            plot_overlay_cell(
                ...
                alpha=alpha_row,
                show_objective_contour=show_objective_contour,
                show_sum_zero_constraint=show_sum_zero_constraint,
            )
...
def main():
    ...
    parser.add_argument(
        "--no_qp_diamond_overlay",
        action="store_true",
        help="Disable objective contours, u+v=0 constraint line, and extra axis padding (for unrelated problems/slices).",
    )
    parser.add_argument(
        "--axis_expand_scale",
        type=float,
        default=1.08,
        help="When QP overlay is on: multiply the span (from lower floor to upper) by this factor (>1 adds padding above samples).",
    )
    parser.add_argument(
        "--axis_lower_floor",
        type=float,
        default=-0.5,
        help="When QP overlay is on: fixed lower axis limit for both coordinates (upper limit follows sample maxima).",
    )
    parser.add_argument(
        "--axis_upper_margin_frac",
        type=float,
        default=0.05,
        help="When QP overlay is on: after span = max(sample)-floor, multiply span by (1 + this fraction).",
    )
    ...
    print(f"Using GT condition_seeds: {selected_seeds} (indices: {[all_seeds.index(s) for s in selected_seeds]})")
    ...
    plot_gt_and_test_methods(
        ...
        xlabel=args.x_label,
        ylabel=args.y_label,
        qp_diamond_overlay=not args.no_qp_diamond_overlay,
        axis_expand_scale=max(float(args.axis_expand_scale), 1.0),
        axis_lower_floor=float(args.axis_lower_floor),
        axis_upper_margin_frac=float(args.axis_upper_margin_frac),
    )
