...
def compute_gradient_and_objective(x, problem_parameters):
    ...
    n = x_torch.shape[0]
    # w_i = 1 + (x_i - alpha - 1) / 4, i = 1..n (parameter alpha from condition vector).
    lambda_1 = lambda_torch[0]


    w = 1.0 + (x_torch - lambda_1 - 1.0) / 4.0

    # f = sin^2(pi w_1) + sum_{i=1}^{n-1} (w_i-1)^2 (1 + 10 sin^2(pi w_i + 1))
    #       + (w_n - 1)^2 (1 + sin^2(2 pi w_n)).
    first = torch.sin(torch.pi * w[0]).pow(2)
    w_head = w[:-1]
    mid = (
        (w_head - 1.0).pow(2)
        * (1.0 + 10.0 * torch.sin(torch.pi * w_head + 1.0).pow(2))
    ).sum()
    last = (w[-1] - 1.0).pow(2) * (1.0 + torch.sin(2.0 * torch.pi * w[-1]).pow(2))

    objective_value = first + mid + last
...
def solve_problem_and_get_k_neighbor_idx(x_i, problem_parameters, error_type="tolerance", error_tolerance=1e-3):
    ...
    return last_iter_idx, convergence_flag
    ...
    for x, lambda_value in zip(x_list, lambda_value_list):
        ...
        for i in range(batch_num):
            ...
            for j in range(sample_num):
                ...
                k_neighbor_idx, convergence_flag = solve_problem_and_get_k_neighbor_idx(
                    x_i, problem_parameters, error_type=error_type, error_tolerance=error_tolerance
                )

                k_neighbor_list.append(k_neighbor_idx)
                converged_list.append(convergence_flag)

    return {"k_neighbors": k_neighbor_list, "converged": converged_list}
...
if __name__ == "__main__":
    ...
    for idx, (input_path, output_path) in enumerate(zip(args.file_paths, args.output_paths)):
        ...
        out = collect_k_neighbor_results(
            input_path,
            problem_type=args.problem_type,
            error_type=args.error_type,
            error_tolerance=args.error_tolerance,
            chain_coupling_weight=args.chain_coupling_weight,
        )
        k_neighbors = out["k_neighbors"]
        ...
        with open(output_path, "wb") as f:
            pickle.dump({"k_neighbors": k_neighbors, "converged": converged}, f)
        
        print(f"Saved {len(k_neighbors)} k-neighbor results ({n_conv} converged per scipy success) to {output_path}")
