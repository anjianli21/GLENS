...
# Hard-coded W&B run names (task = himmelblau; same order as the 4 runs below).
wandb_run_name_list=(
  "himmelblau_diffusolve_k_1"
  "himmelblau_diffusolve_k_10"
  "himmelblau_ns_k_10"
  "himmelblau_glense_k_10"
)

# Placeholders (edit these before running)
# NOTE: The 4 test-sample runs are ordered as:
#   (1) Diffusolve  k=1
#   (2) Diffusolve  k=10
#   (3) NS          k=10
...
condition_encoder_data_type_strings_list=("parameter_minmax_raw_concat_64"
"parameter_minmax_raw_concat_64" "parameter_minmax_radius_scale1_raw_concat_64"
"parameter_minmax_radius_scale1_raw_concat_64")
condition_encoder_data_type_list=('parameter' 'parameter' 'parameter,radius' 'parameter,radius')
condition_y_parameter_dim_list=(2 2 2 2)
unet_dim_list=(64 64 64 64)
unet_dim_mults_list=('(1,2,4)' '(1,2,4)' '(1,2,4)' '(1,2,4)')
embed_y_all_dim_list=(64 64 64 64)
...
parameter_data_min_list=(1 1 1 1)
parameter_data_max_list=(50 50 50 50)
radius_data_process_type_list=('scale' 'scale' 'scale' 'scale')
radius_data_scale_list=(1 1 1 1)
...
solver_behavior_condition_y_parameter_dim_list=(2 2 2 2)
...
for i in "${!wandb_run_name_list[@]}"; do
    ...
    parameter_data_min=${parameter_data_min_list[$i]}
    parameter_data_max=${parameter_data_max_list[$i]}
    ...
    python run/test/diffusion/test_diffusion_sample_with_solver_behavior.py \
        --task_name himmelblau \
        --project_name "${project_name}" \
        --result_folder "${LOGS_DIR}/himmelblau/lbfgsb" \
        --wandb_mode offline \
        ...
        --parameter_data_min "${parameter_data_min}" \
        --parameter_data_max "${parameter_data_max}" \
