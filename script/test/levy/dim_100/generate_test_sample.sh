...
# Hard-coded W&B run names (task = levy; same order as the 4 runs below).
wandb_run_name_list=(
  "levy_diffusolve_k_1"
  "levy_diffusolve_k_10"
  "levy_ns_k_10"
  "levy_glense_k_10"
)
...
"parameter_minmax_raw_concat_32" "parameter_minmax_radius_scale01_raw_concat_32"
"parameter_minmax_radius_scale01_raw_concat_32")
...
parameter_data_min_list=('-2.0' '-2.0' '-2.0' '-2.0')
parameter_data_max_list=('2.0' '2.0' '2.0' '2.0')
radius_data_process_type_list=('scale' 'scale' 'scale' 'scale')
radius_data_scale_list=(0.1 0.1 0.1 0.1)
...
for i in "${!wandb_run_name_list[@]}"; do
    ...
    python run/test/diffusion/test_diffusion_sample_with_solver_behavior.py \
        ...
        --result_folder "${LOGS_DIR}/levy/lbfgsb" \
        --wandb_mode offline \
