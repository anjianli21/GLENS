...
# Hard-coded W&B run names (task = rosenbrock; same order as the 4 runs below).
wandb_run_name_list=(
  "rosenbrock_diffusolve_k_1"
  "rosenbrock_diffusolve_k_10"
  "rosenbrock_ns_k_10"
  "rosenbrock_glense_k_10"
)
...
parameter_data_min_list=('50,-1.5' '50,-1.5' '50,-1.5' '50,-1.5')
parameter_data_max_list=('200,0.0' '200,0.0' '200,0.0' '200,0.0')
...
for i in "${!wandb_run_name_list[@]}"; do
    ...
    python run/test/diffusion/test_diffusion_sample_with_solver_behavior.py \
        --task_name rosenbrock \
        --project_name "${project_name}" \
        --result_folder "${LOGS_DIR}/rosenbrock/lbfgsb" \
        --wandb_mode offline \
        ...
        --condition_y_parameter_dim "${solver_behavior_condition_y_parameter_dim}" \
